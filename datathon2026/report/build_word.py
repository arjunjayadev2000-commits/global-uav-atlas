"""Editable Word (.docx) versions of the three PDFs.

The PDF builders (build_report.py, build_commander.py, build_scoresheet.py) save their final HTML in report/_html/.
...
def build_scoresheet():
    s = BeautifulSoup((HTML / "scoresheet.html").read_text(), "html.parser")
    cv = Converter(css_of(s), "CDM Datathon-2026  |  Self-Assessment against Gen Instr Appendix B", margins=(15, 15, 15, 15))
    cv.furniture(cv.doc.sections[0], "decimal")
    for el in s.body.children:
        if isinstance(el, Tag) and el.name == "h1":
            cv.para(node=el, style="Front Heading")
        elif isinstance(el, Tag) and ("tot" in el.get("class", []) or "note" in el.get("class", [])):
            cv.box(el, "callout" if "tot" in el.get("class", []) else "plain")
        elif isinstance(el, Tag) and "sub" in el.get("class", []):
            cv.para(node=el, align=WD_ALIGN_PARAGRAPH.CENTER, italic=True)
        else:
            cv.block(el)
    cv.save(OUT / "Datathon2026_Self_Assessment_Sheet.docx")


if __name__ == "__main__":
    build("commander", "CDM Datathon-2026  |  Commander's Edition  |  Global Conflicts - Impact on Supply Chains",
          chapter_breaks=False, contents_levels=1, margins=(20, 18, 20, 18), out="Datathon2026_Commanders_Edition.docx")
    build("report", "CDM Datathon-2026  |  Global Conflicts - Impact on Supply Chains",
          chapter_breaks=True, contents_levels=3, margins=(20, 18, 22, 18), out="Datathon2026_Global_Conflicts_Supply_Chains_Report.docx")
    build_scoresheet()
