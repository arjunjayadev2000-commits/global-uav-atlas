...
keep = [l for l in log if l.startswith("Stage") or "rows" in l] + [f"  ... {sum('->' in l for l in log)} figures and tables written"]
...
src = (ROOT / "analysis/08_intel.py").read_text().splitlines()
start = next(i for i, l in enumerate(src) if l.startswith("def rr_boot"))
...
with sync_playwright() as p:
    ...
    shot(pg, WIN.format(bg="#1e1e1e", fg="#e5e5e5", title="Terminal - datathon2026 - full pipeline run (raw data to report)", body=term),
         FIG / "screen_pipeline.png")
    shot(pg, WIN.format(bg="#263238", fg="#eeffff", title="analysis/08_intel.py - scene-cluster bootstrap for the robustness test", body=body),
         FIG / "screen_code.png")
    br.close()
